...
void cOffset::GameInfo(void)
{
	...
	__asm // íó êóäà â c++ áåç asm ? D:
	{
		call Address // à òåïåðü âûçûâàåì ôóíêöèþ êîòîðîÿ âîçâðàùÿåò build
		mov BuildInfo.Build, eax // íó è return ñåáå ïèõàåì
	}
}
...
// ben ekledim ADJUST DENEN SEVDÝGÝMÝN FONKSÝYONU ÝÇÝN!!
void* cOffset::SpeedHackPtr(void)
{
	...
	void* SpeedPtr = (void*)*(DWORD*)(FindReferencee(HwBase, HwBase + HwSize, Address) - 7); //findreferencee yaptým 
...
{
	DWORD Address = FindPatternbenimki("\x56\x57\x33\xFF\x3B\xC7\x0F\x84\x00\x00\x00\x00\x83\xF8\x01\x0F\x84\x00\x00\x00\x00\x83\xF8\x02\x0F\x84\x00\x00\x00\x00\x83\xF8\x03\x75\x22", "xxxxxxx????xxxxx????xxxxx????xxxxx", HwBase, HwEnd, 0);

	if (FarProc((DWORD)Address, HwBase, HwEnd))
		Error("CL_Move: not found.");
	else {
		Address = FindPatternbenimki("\xC3\x90", "xx", Address - 0x12, HwEnd, 0x2);
...
{
	dwSendPacketPointer = FindPatternbenimki("\x75\x13\xD9\x05\x00\x00\x00\x00\xD8\x1D\x00\x00\x00\x00\xDF\xE0\xF6\xC4\x00\x00\x00\xD9\x05\x00\x00\x00\x00\xDC\x1D\x00\x00\x00\x00\xDF\xE0\xF6\xC4\x41", "xxxx????xx????xxxx???xx????xx????xxxxx", HwBase, HwEnd, 0x1b) + 2;
...
PClientUserMsg cOffset::FindUserMsgBase(void)
{
...
{
	DWORD Address = FindPatternbenimki("ScreenFade", HwBase, HwEnd, 0);
	PVOID Ptr = (PVOID) * (PDWORD)(FindReferencee(HwBase, HwEnd, Address) + 0x18);
...
DWORD cOffset::FindPatternbenimki(PCHAR pattern, PCHAR mask, DWORD start, DWORD end, DWORD offset)
{
	if (start > end)
	{
		DWORD reverse = end;
...
DWORD cOffset::FindPatternbenimki(PCHAR pattern, DWORD start, DWORD end, DWORD offset)
{
	if (start > end)
	{
		DWORD reverse = end;
...
bool cOffset::EnablePageWrite(DWORD addr, DWORD size) //korumayý deðiþtirme
{
	return VirtualProtect((void*)addr, size, PAGE_EXECUTE_READWRITE, &dwOldPageProtection) != 0;
}

bool cOffset::RestorePageProtection(DWORD addr, DWORD size) //eski korumayý geri verme
{
